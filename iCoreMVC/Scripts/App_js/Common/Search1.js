function Search() {
    ...
    this.InitializeSearch = function () {
        ...
        window.parent.$('#btnMultiSelect').click(function () {
            GetSelectedData(null);
        });
        window.parent.$('#btnApprove').click(function () {
            GetSelectedData(true);
        });
        window.parent.$('#btnReject').click(function () {
    ...
    function GetSelectedData(approveorrejected) {
        var SelectedData = [];
        for (var i = 0; i < multiSelectData.length; i++) {
            for (var j = 0; j < multiSelectData[i].RowData.length; j++) {
                SelectedData.push(multiSelectData[i].RowData[j]);
            }
        }
        var selRows = $("#JQGridForSearch").getGridParam('selarrrow');
        for (var i = 0; i < selRows.length; i++) {
            var data = $("#JQGridForSearch").jqGrid('getRowData', selRows[i]);
            var isExists = IsElementExists(SelectedData, data);
            if (!isExists) {
        ...
        try {
            if (SelectedData.length > 0) {
                var data = { rows: SelectedData, serachid: GetQueryStringParams('SearchID'), approveorreject: approveorrejected };
                var window_opener = window.dialogArguments;
                window.parent.GetSelectedData(data);
                window.parent.UnloadModalDialog();
            }
            else {
                window.parent.Alerterror('Please select atleast one record');
            }
        }
        catch (err) { }
    }
    ...
    function resizeModelDialog(height) {
        try {
           ...
           CommonSearchResizeHeight('modal-dialog-frame', (height + 10));
        }
        catch (err) {
            ...
            for (var i = 0; i < ids.length; i++) {
                if ($(this).attr('id').indexOf(ids[i]) < 0) {
                    if ($('#' + ids[i] + '_field').val() == $(this).val()) {
                        $(this).val('-1');
                        window.parent.Alerterror('Duplicate search condition. Please select a different one');
                        return;
                    }
                }
            }
